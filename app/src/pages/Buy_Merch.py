        ...
        with col_b:
            if st.button("Place Order", use_container_width=True, type="primary"):
                ...
                if success_count == get_cart_count():
                    st.success(
                        f"🎉 Order placed successfully! {success_count} items ordered."
                    )
                    clear_cart()
                    st.session_state.checkout_mode = False
                    st.rerun()
...
if (
    ...
    or st.session_state["role"] == "vp"
    or st.session_state["role"] == "administrator"
):
    with st.expander("🔧 Admin Panel"):
        ...
        with col2:
            new_item_price = st.number_input(
                "Price", min_value=0.0, format="%.2f", key="admin_price"
            )
        with col3:
            new_item_quantity = st.number_input(
                "Quantity", min_value=0, key="admin_quantity"
            )
        ...
        try:
            ...
            if sales_response.status_code == 200:
                ...
                if sales_data:
                    ...
                    total_revenue = sum(
                        float(sale.get("TotalSalePrice", 0)) for sale in sales_data
                    )
                    cash_sales = sum(
                        1 for sale in sales_data if float(sale["Cash"]) > 0
                    )
                        ...
                        for sale in sales_data:
                            try:
                                ...
                                if date_filter == "Today" and sale_date == today:
                                    ...
                                    and sale_date >= today - timedelta(days=7)
                                ):
                                    filtered_sales.append(sale)
                                elif (
                                    date_filter == "This Month"
                                    and sale_date >= today - timedelta(days=30)
                                ):
                                    filtered_sales.append(sale)
                            except:
                    ...
                    if payment_filter == "Cash Only":
                        filtered_sales = [
                            sale for sale in filtered_sales if float(sale["Cash"]) > 0
                        ]
                    ...
                    if filtered_sales:
                        ...
                        items_sold = Counter(
                            sale["ItemsSold"] for sale in filtered_sales
                        )
                        ...
                        with col_table:
                            ...
                            for sale in sorted(
                                ...
                                try:
                                    ...
                                    formatted_date = sale_date.strftime(
                                        "%m/%d/%Y %H:%M"
                                    )
                                except:
                                ...
                                payment_type = (
                                    "💵 Cash" if float(sale["Cash"]) > 0 else "💳 Card"
                                )
                                sale_price = float(sale.get("TotalSalePrice", 0))

                                col_item, col_payment, col_price, col_date = st.columns(
                                    [2, 1, 1, 2]
                                )
                                with col_item:
                        ...
                        with col_chart:
                            ...
                            for item, count in items_sold.most_common(5):
                                ...
                                # Simple progress bar
                                progress = (
                                    count / max(items_sold.values())
                                    if items_sold.values()
                                    else 0
                                )
                    ...
                    with col_export1:
                        if st.button("📥 Download CSV", use_container_width=True):
                            ...
                            for index, row in df.iterrows():
                                try:
                                    sale_date = datetime.strptime(
                                        row["SaleDate"], "%a, %d %b %Y %H:%M:%S %Z"
                                    )
                                    df.at[index, "SaleDate"] = sale_date.strftime(
                                        "%Y-%m-%d %H:%M:%S"
                                    )
                                except:
                                    pass

                                df.at[index, "PaymentMethod"] = (
                                    "Cash" if float(row["Cash"]) > 0 else "Card"
                                )
                    ...
                    with col_export2:
                        ...
                        filtered_revenue = sum(
                            float(sale.get("TotalSalePrice", 0))
            ...
            for j, item in enumerate(merch_items[i : i + cols_per_row]):
                with cols[j]:
                    ...
                    with st.container():
                        ...
                        st.image("https://placehold.co/400", use_container_width=True)
                        ...
                        if st.session_state.get("first_name") == "Jacob":
                            with col_admin:
                                if st.button(
                                    "🗑️", key=f"delete_{item['ID']}", help="Delete item"
                                ):
                                    try:
                                        response = requests.delete(
                                            f"{BASE_URL}/merch/{item['ID']}"
                                        )
                                        if response.status_code == 200:
                                            st.success("Deleted!")
                                            st.cache_data.clear()
