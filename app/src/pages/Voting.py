    ...
    if unvoted_ballots:
        ...
        for ballot in unvoted_ballots:
            with st.container():
                ...
                try:
                    ballot_response = requests.get(f"{BASE_URL}/elections/ballots/{ballot['BallotID']}")
                    if ballot_response.status_code == 200:
                        ...
                        if options:
                            ...
                            with st.form(f"vote_form_{ballot['BallotID']}"):
                                ...
                                candidate_options = {f"{opt['CandidateName']}": opt['OptionID'] for opt in options}
                                ...
                                if submit_vote:
                                    if selected_candidate and selected_candidate != "":
                                        selected_option_id = candidate_options[selected_candidate]
                                        
                                        # Confirm vote
                                        if st.session_state.get(f"confirm_vote_{ballot['BallotID']}", False):
                                            # Submit the vote
                                            vote_data = {
                                                "member_id": member_id,
                                            ...
                                            try:
                                                ...
                                                if vote_response.status_code == 201:
                                                    st.success(f"✅ Vote cast successfully for {ballot['PositionTitle']}!")
                                                    st.balloons()
                                                    
                                                    # Clear confirmation state
                                                    del st.session_state[f"confirm_vote_{ballot['BallotID']}"]
                                                    
                                                    # Refresh page
                                                    st.rerun()
                                                else:
                                                    error_msg = vote_response.json().get('error', 'Unknown error')
                                                    st.error(f"Error casting vote: {error_msg}")
                                            except Exception as e:
                                                st.error(f"Could not submit vote: {e}")
                                        else:
                                            # Show confirmation
                                            st.warning(f"⚠️ Confirm your vote for **{selected_candidate}** in {ballot['PositionTitle']}")
                                            st.session_state[f"confirm_vote_{ballot['BallotID']}"] = True
                                            st.rerun()
                                    else:
                                        st.error("Please select a candidate before voting.")
                        else:
