    ...
    with setup_tab2:
        ...
        with col1:
            ...
            with st.form("create_position"):
                position_title = st.text_input("Position Title", placeholder="e.g., President")
        ...
        with st.form("create_election"):
            ...
            with col1:
                ...
                try:
                    ...
                    terms = terms_response.json() if terms_response.status_code == 200 else []
                    term_options = {term['Name']: term['ID'] for term in terms}
                    selected_term = st.selectbox("Term", options=list(term_options.keys()))
            ...
            with col2:
                ...
                try:
                    ...
                    selected_positions = st.multiselect("Positions", options=list(position_options.keys()))
                except:
            ...
            if st.form_submit_button("Create Election", use_container_width=True):
                if selected_term and selected_positions:
                    data = {
                        ...
                        "positions": [position_options[pos] for pos in selected_positions],
...
with tab3:
    ...
    with nom_tab1:
        ...
        with st.form("submit_nomination"):
            ...
            with col1:
                nominator_id = st.number_input("Nominator Member ID", min_value=1, step=1)
            ...
            with col3:
                ...
                try:
                    ...
                    positions = positions_response.json() if positions_response.status_code == 200 else []
                    position_options = {pos['Title']: pos['ID'] for pos in positions}
                    selected_position = st.selectbox("Position", options=list(position_options.keys()))
                except:
                    st.error("Could not load positions")
    ...
    with nom_tab2:
        ...
        try:
            ...
            elections = elections_response.json() if elections_response.status_code == 200 else []
            
            if elections:
                election_options = {f"{e['TermName']} - {e['Date']}": e['ID'] for e in elections}
                selected_election = st.selectbox("Select Election", options=list(election_options.keys()))
                
                if selected_election:
                    election_id = election_options[selected_election]
                    
                    # Get nominations for selected election
                    noms_response = requests.get(f"{API_BASE}/nominations/election/{election_id}")
                                ...
                                with col4:
                                    ...
                                    if status is None:
                                        ...
                                        if st.button("Accept", key=f"accept_{nom['ID']}", use_container_width=True):
                                            ...
                                            accept_response = requests.put(f"{API_BASE}/nominations/{nom['ID']}/accept", json=accept_data)
                                            if accept_response.status_code == 200:
                                                st.rerun()
                                        if st.button("Decline", key=f"decline_{nom['ID']}", use_container_width=True):
                                            decline_data = {"accepted": False}
                                            decline_response = requests.put(f"{API_BASE}/nominations/{nom['ID']}/accept", json=decline_data)
...
with tab4:
    ...
    with ballot_tab1:
        ...
        try:
            ...
            if elections:
                election_options = {f"{e['TermName']} - {e['Date']}": e['ID'] for e in elections}
                selected_election = st.selectbox("Select Election", options=list(election_options.keys()), key="ballot_election")
                
                col1, col2 = st.columns(2)
                
                with col1:
                    if st.button("Generate Ballots", use_container_width=True, type="primary"):
                        election_id = election_options[selected_election]
                        try:
                            response = requests.post(f"{API_BASE}/elections/{election_id}/generate-ballots")
                            if response.status_code == 201:
                                result = response.json()
                                st.success(f"Generated {len(result['ballots'])} ballots successfully!")
                                for ballot in result['ballots']:
                                    st.write(f"- {ballot['position']}: {ballot['options_count']} candidates")
                            else:
                                st.error(f"Error: {response.json().get('error', 'Unknown error')}")
                        except:
                            st.error("Could not connect to API")
                ...
                with col2:
                    st.info("This will create ballots for all positions with accepted nominations that don't already have winners.")
        except:
    ...
    with ballot_tab2:
        ...
        member_id = st.number_input("Enter Member ID to view their ballots", min_value=1, step=1, key="voting_member")
...
with tab5:
    ...
    with result_tab1:
        ...
        ballot_id = st.number_input("Enter Ballot ID", min_value=1, step=1, key="results_ballot")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for candidate in results['results']:
                        ...
                        with col3:
                            percentage = (candidate['VoteCount'] / results['total_votes'] * 100) if results['total_votes'] > 0 else 0
    ...
    with result_tab2:
        ...
        col1, col2 = st.columns(2)
        with col1:
            ballot_id_winner = st.number_input("Ballot ID", min_value=1, step=1, key="winner_ballot")
        with col2:
            winner_member_id = st.number_input("Winner Member ID", min_value=1, step=1, key="winner_member")
        
        if st.button("Declare Winner", type="primary"):
            data = {"member_id": winner_member_id}
            try:
                response = requests.post(f"{API_BASE}/ballots/{ballot_id_winner}/declare-winner", json=data)
                if response.status_code == 201:
...
with tab6:
    ...
    try:
        ...
        if elections:
            election_options = {f"{e['TermName']} - {e['Date']}": e['ID'] for e in elections}
            selected_election = st.selectbox("Select Election for Report", options=list(election_options.keys()), key="report_election")
            
            if selected_election:
                election_id = election_options[selected_election]
                
                # Get winners for this election
                winners_response = requests.get(f"{API_BASE}/elections/{election_id}/winners")
                ...
                nominations_response = requests.get(f"{API_BASE}/nominations/election/{election_id}")
                if nominations_response.status_code == 200:
                    ...
                    accepted_nominations = len([n for n in nominations if n['Accepted'] is True])
                    pending_nominations = len([n for n in nominations if n['Accepted'] is None])
