...
if is_treasurer:
        ...
        if not reimbursement_data:
            ...
            with col_filter2:
                ...
                for member_id in member_ids:
                    try:
                        ...
                        if resp.status_code == 200:
                            data = resp.json()
                            member_data = data.get(
                                "member", {}
                            )  # Extract the member object
                            # Prefer PreferredName if available, else FirstName
                            first_name = member_data.get(
                                "PreferredName"
                            ) or member_data.get("FirstName", "")
                ...
                selected_member = st.selectbox(
                    "Filter by Member", options=member_options
                )
            ...
            if len(filtered_df) > 0:
                    ...
                    with st.container():
                        ...
                        with col2:
                            ...
                            if (
                                ...
                                st.write(
                                    f"📝 {row['Description'][:50]}{'...' if len(str(row['Description'])) > 50 else ''}"
                                )
                            else:
                                ...
                                if st.button(
                                    ...
                                    try:
                                        response = requests.put(
                                            f"http://api:4000/reimbursements/{row['ID']}/approve"
                                        )
                                        if response.status_code == 200:
                                            st.success(
                                                f"Approved reimbursement for {row['MemberName']}"
                                            )
                                            st.rerun()
                                        else:
                        ...
                        with col5:
                            ...
                            if st.button(
                                ...
                                st.session_state[f"show_details_{row['ID']}"] = (
                                    not st.session_state.get(
                                        f"show_details_{row['ID']}", False
                                    )
                                )
                ...
                if len(pending_in_filter) > 0:
                    ...
                    with col2:
                        st.info(
                            f"This will approve {len(pending_in_filter)} pending reimbursements in the current filter"
                        )
    ...
    st.error(
        "🚫 Access Denied: Only treasurers and administrators can view reimbursement requests."
    )
    st.info(
        "Please contact your club administrator if you believe you should have access to this page."
    )
