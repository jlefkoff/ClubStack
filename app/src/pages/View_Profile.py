...
with st.expander("Filters", expanded=True):
    ...
    with c2:
        grad_years = sorted({m.get("GraduationYear") for m in members if m.get("GraduationYear")})
        grad_filter = st.multiselect("Graduation Year", grad_years)
    with c3:
        student_type = st.selectbox("Student Type", ["All", "Undergraduate", "Graduate"])
...
cols = [
    ...
    "GraduationYear", "IsGradStudent", "ActivationDate",
    "CarPlate", "CarState", "CarPassCount",
    "EmerContactName", "EmerContactPhone", "Allergies"
]
df = pd.DataFrame(filtered)[[c for c in cols if c in filtered[0].keys()]]
st.dataframe(df, use_container_width=True, hide_index=True)

# ---- Select a member to edit
labels = [f"{m['ID']} — {m.get('FirstName','')} {m.get('LastName','')}" for m in filtered]
...
with st.form("edit_member_form", clear_on_submit=False):
    ...
    with c1:
        ...
        preferred_name = st.text_input("Preferred Name", value=profile.get("PreferredName") or "")
        graduation_year = st.number_input("Graduation Year", value=profile.get("GraduationYear") or 2025, min_value=1900, max_value=2100)
        is_grad_student = st.checkbox("Graduate Student", value=bool(profile.get("IsGradStudent")))
    with c2:
        ...
        car_pass_count = st.number_input("Car Pass Count", value=profile.get("CarPassCount") or 0, min_value=0, step=1)
        emer_contact_name = st.text_input("Emergency Contact Name", value=profile.get("EmerContactName") or "")
        emer_contact_phone = st.text_input("Emergency Contact Phone", value=profile.get("EmerContactPhone") or "")
    ...
    if submitted:
        ...
        try:
            up = requests.put(f"{API_BASE}/members/{selected_id}", json=payload, headers=headers, timeout=15)
...
with colA:
    if st.button("Renew / Activate"):
        try:
            act = requests.put(f"{API_BASE}/members/{selected_id}/activate", headers=headers, timeout=10)
