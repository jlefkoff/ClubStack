...
try:
    ...
    if response.status_code == 200:
        ...
        st.write(
            f"**Emergency Contact:** {profile.get('EmerContactName')} - {profile.get('EmerContactPhone')}"
        )
        ...
        with st.form("edit_member_form"):
            ...
            preferred_name = st.text_input(
                "Preferred Name", value=profile.get("PreferredName", "")
            )
            ...
            is_grad_student = st.checkbox(
                "Graduate Student", value=profile.get("IsGradStudent", False)
            )
            ...
            car_pass_count = st.number_input(
                "Car Pass Count", value=profile.get("CarPassCount") or 0, min_value=0
            )

            emer_contact_name = st.text_input(
                "Emergency Contact Name", value=profile.get("EmerContactName", "")
            )
            emer_contact_phone = st.text_input(
                "Emergency Contact Phone", value=profile.get("EmerContactPhone", "")
            )
            ...
            if st.form_submit_button("Update Member"):
                ...
                try:
                    ...
                    update_response = requests.put(
                        f"{API_BASE}/members/{member_id}", json=update_data
                    )
