            ...
            if members_response.status_code == 200:
                ...
                with col_a:
                    if st.button(
                        ...
                        st.session_state.selected_members = [
                            m["ID"] for m in all_members if isinstance(m, dict)
                        ]
                        st.rerun()
                with col_b:
                    if st.button(
                        "Clear All", type="secondary", use_container_width=True
                    ):
                        st.session_state.selected_members = []
                        st.rerun()
                ...
                with st.form("send_communication"):
                    ...
                    with col1:
                        subject = st.text_input(
                            "Subject", placeholder="Enter message subject..."
                        )
                        content = st.text_area(
                            "Message", placeholder="Enter your message...", height=150
                        )
                    ...
                    with col2:
                        ...
                        if role_filter != "All":
                            filtered_members = [
                                ...
                                and m.get("Role", "member") == role_filter
                            ]
                        else:
                        ...
                        with st.container():
                            if len(filtered_members) > 0:
                                ...
                                for member in sorted(
                                    filtered_members,
                                    key=lambda x: f"{x.get('FirstName', '')} {x.get('LastName', '')}",
                                ):
                                    if isinstance(member, dict):
                                        member_name = f"{member.get('FirstName', 'Unknown')} {member.get('LastName', 'User')}"
                                        is_selected = (
                                            member["ID"]
                                            in st.session_state.selected_members
                                        )
                                        ...
                                        if (
                                            selected
                                            and member["ID"]
                                            not in st.session_state.selected_members
                                        ):
                                            st.session_state.selected_members.append(
                                                member["ID"]
                                            )
                                        elif (
                                            not selected
                                            ...
                                            st.session_state.selected_members.remove(
                                                member["ID"]
                                            )
                            else:
                        ...
                        st.write(
                            f"**Selected: {len(st.session_state.selected_members)} members**"
                        )
                        ...
                        if not subject or not content:
                            ...
                            try:
                                ...
                                if response.status_code == 201:
                                    st.success(
                                        f"✅ Message sent successfully to {len(st.session_state.selected_members)} members!"
                                    )
                                    # Clear form
                                    st.session_state.selected_members = []
                                else:
        ...
        try:
            ...
            if messages_response.status_code == 200:
                ...
                if messages:
                    ...
                    st.write(
                        f"**Showing {len(filtered_messages)} of {len(messages)} messages**"
                    )
                    st.write("---")
                    ...
                    for message in filtered_messages:
                        ...
                        try:
                            msg_date = datetime.strptime(
                                message["DateSent"], "%a, %d %b %Y %H:%M:%S %Z"
                            )
                            ...
                            if st.button(
                                ...
                                try:
                                    delete_response = requests.delete(
                                        f"{BASE_URL}/communications/{message['ID']}"
                                    )
    ...
    try:
        ...
        if messages_response.status_code == 200:
            ...
            if messages:
                ...
                search_term = st.text_input(
                    "🔍 Search messages...", placeholder="Search by subject or content"
                )
...
if is_admin:
    st.info(
        "💡 **Admin Tip**: You can send messages to specific members or all members at once. Use role filters to target specific groups."
    )
else:
    st.info(
        "💡 **Tip**: This is where you'll receive important club announcements and updates from leadership."
    )
