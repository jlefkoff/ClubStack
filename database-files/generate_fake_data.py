...
def generate_fake_data():
    ...
    sql_statements.append("-- Run this file against your ClubStack database to populate with fake data")
    ...
    for member_id in member_ids:
        ...
        activation_date = fake.date_between(start_date='-2y', end_date='today').strftime('%Y-%m-%d')
    ...
    common_allergies = [
        'Shellfish', 'Fish', 'Eggs', 'Soy', 'Sesame', 'Sulfites', 
        'Latex', 'Dust Mites', 'Pet Dander', 'Pollen', 'Bee Stings',
        'Penicillin', 'Aspirin', 'Strawberries', 'Chocolate'
    ]
    ...
    event_names = [
        'Beginner Rock Climbing', 'Advanced Mountaineering', 'Weekend Backpacking',
        'Map and Compass Workshop', 'Wilderness First Aid', 'Gear Maintenance Session',
        'New Member BBQ', 'Monthly Social Dinner', 'Film Night: Mountain Movies',
        'Knot Tying Workshop', 'Leave No Trace Training', 'Winter Camping Prep',
        'Avalanche Safety Course', 'Photography Workshop', 'Trail Maintenance Day',
        'Club Fair Booth', 'Officer Elections', 'Budget Planning Meeting',
        'Gear Sale Event', 'Trip Planning Session', 'Safety Committee Meeting',
        'White Mountain Day Hike', 'Ice Climbing Introduction', 'Ski Touring Basics',
        'Camping Fundamentals', 'Peak Bagging Challenge', 'Sunrise Photography Hike'
    ]
    
    locations = ['White Mountains NH', 'Loj', 'Pub', 'Centennial', 'Campus', 'Local Crag', 'Gear Room', 'Marino Center']
    meet_locations = ['Curry Student Center', 'Marino Center', 'Ruggles Station', 'Back Bay Station', 'Campus Quad']
    organizations = ['Hiking Committee', 'Climbing Committee', 'Social Committee', 'Safety Committee', 'Membership Committee']
    event_types = ['Outdoor', 'Educational', 'Social', 'Training', 'Meeting', 'Competition']
    ...
    for event_id in event_ids:
        ...
        for member_id in event_members:
            date_registered = fake.date_time_between(start_date='-30d', end_date='now').strftime('%Y-%m-%d %H:%M:%S')
    ...
    for _ in range(150):  # 150 feedback entries
        ...
        rating = random.choices([1, 2, 3, 4, 5], weights=[2, 5, 15, 35, 43])[0]  # Mostly positive
    ...
    for _ in range(25):  # 25 communications
        ...
        date_sent = fake.date_time_between(start_date='-6m', end_date='now').strftime('%Y-%m-%d %H:%M:%S')
    ...
    vendor_names = [
        'REI Co-op', 'Eastern Mountain Sports', 'Outdoor Gear Exchange',
        'International Mountain Equipment', 'Climb High', 'Backcountry Gear',
        'Local Outdoor Store', 'Mountain Warehouse', 'Gear Coop',
        'Adventure Outpost', 'Summit Sports', 'Basecamp Outfitters'
    ]
    ...
    reimbursement_types = ['Equipment', 'Transportation', 'Food', 'Training', 'Event Supplies', 'Maintenance']
    ...
    for po_id in range(1, 16):  # Purchase Orders 1-15
        ...
        reimbursement_id = random.choice(reimbursement_ids) if random.random() < 0.8 else 'NULL'
    ...
    gear_items = [
        ...
        ('Climbing Shoes', 85.00, ['6', '7', '8', '9', '10', '11', '12'], 'Gear Room A'),
    ...
    for _ in range(100):  # 100 rental items
        ...
        availability = "Available for checkout" if status == 'AVAILABLE' else f"Status: {status}"
    ...
    for _ in range(40):  # 40 merch items
        ...
        purchase_order = random.randint(1, 15)  # Reference existing POs (1-5 from seed, 6-15 from fake data)
